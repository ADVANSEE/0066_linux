...
static struct reg_value ov7251_init_setting_100fps_VGA[] = {
    ...
    {0x30b0,0x0a,0,0},{0x30b1,0x01,0,0},{0x30b3,0x64,0,0},
    ...
    {0x3636,0x00,0,0},{0x3662,0x01,0,0},{0x3663,0x70,0,0},
...
int OV7251_get_gain16(void)
{
	 /* read gain, 16 = 1x */
	int gain16;
	u8 temp;

	gain16 = ov7251_read_reg(0x350a, &temp) & 0x03;
	gain16 = (gain16<<8) + ov7251_read_reg(0x350b, &temp);

	return gain16;
}

int OV7251_set_gain16(int gain16)
{
	/* write gain, 16 = 1x */
	u8 temp;
	gain16 = gain16 & 0x3ff;
	...
	temp = gain16 & 0xff;
	ov7251_write_reg(0x350b, temp);

	temp = gain16>>8;
	ov7251_write_reg(0x350a, temp);
...
static int ov7251_init_mode( enum ov7251_mode mode)
{
	...
	if (mipi_csi2_info) {
		...
		if (mipi_csi2_get_status(mipi_csi2_info)) {
			...
			{
			      pr_err("mipi_csi2_reset\n");
			      mipi_csi2_reset(mipi_csi2_info);
			}
			if (ov7251_data.pix.pixelformat == V4L2_PIX_FMT_Y10)
			{
	...
	pr_err("ov7251 set virtual channel\n");
	...
	if (mipi_csi2_info) {
		...
		pr_err("mipi receive data correctly\n");
	}
err:
	return retval;
}
			...
			const struct i2c_device_id *id)
{
	...
	ov7251_data.pix.pixelformat = V4L2_PIX_FMT_Y10;
	ov7251_data.pix.width = 640;
	ov7251_data.pix.height = 480;
